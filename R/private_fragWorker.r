...
#' @param i Integer, block number of \code{rast}.
...
.fragWorker <- compiler::cmpfun(function(
	...
	# initialize
	if (calcDensity | calcClass) blockDensity <- NA * rast
	if (calcConnect | calcClass) blockConnect <- NA * rast
	if (calcClass) blockClass <- NA * rast
	
	# by row and column
	for (countRow in 1:stopAtRow) {
		for (countCol in 1:ncol(rast)) {
			
			# window limits
			topRow <- max(1, countRow - halfWindowSize)
			bottomRow <- countRow + halfWindowSize
			leftCol <- max(1, countCol - halfWindowSize)
			rightCol <- min(ncol(rast), countCol + halfWindowSize)
			thisWindow <- rast[topRow:bottomRow, leftCol:rightCol]
			
			# calculate density, connectivity, and class
			if (calcDensity) blockDensity[countRow, countCol] <- .fragDensity(rast=thisWindow, na.rm=na.rm)
			if (calcConnect) blockConnect[countRow, countCol] <- .fragConnect(rast=thisWindow, na.rm=na.rm)
