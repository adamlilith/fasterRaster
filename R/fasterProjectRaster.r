...
#'
#' This function is a potentially faster version of the \code{\link[terra]{projectRaster}} function in the \pkg{terra} package for projecting (and resampling) a raster. It requires the user has GRASS GIS Version 7 installed.
...
#' @param grassToR Logical, if \code{TRUE} (default) then the output will be returned to R. If \code{FALSE}, then the output is left in the GRASS session and named the value in \code{outGrassName} \code{rast}. The latter case is useful (and faster) when chaining several \pkg{fasterRaster} functions together.
...
#' @param ... Arguments to pass to \code{\link[rgrass]{execGRASS}} when used for rasterizing (i.e., function \code{r.proj} in GRASS).
