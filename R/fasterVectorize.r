...
#'
#' This function is a potentially faster version of the function \code{\link[terra]{rasterToPolygons}} in the \pkg{terra} package. It can convert a raster to points or polygons (conversion to lines is not yet supported, although it is possible using the \code{r.to.vect} module in GRASS).
...
#' @param grassToR Logical. If \code{TRUE} (default) then the product of the calculations will be returned to R. If \code{FALSE}, then the output is left in the GRASS session and named the value in \code{outGrassName} \code{rastToVect}. The latter case is useful (and faster) when chaining several \pkg{fasterRaster} functions together.
...
#' @param ... Arguments to pass to \code{\link[rgrass]{execGRASS}} when used for converting a raster to a vector (i.e., function \code{r.to.vect} in GRASS).
...
#' @details See the documentation for the GRASS module \code{r.to.vect}{https://grass.osgeo.org/grass82/manuals/r.to.vect.html}.
...
fasterVectorize <- function(
	...
	if (grassToR) {
		...
		if (agg) out <- raster::aggregate(out, by='value')
		out
