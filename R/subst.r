...
#' * `from` and `to` are character vectors: You can use a character vector for `from` and `to`. In this case, the input raster must be a [factor][tutorial_raster_data_types] (categorical) raster or an integer raster. If `from` is a character vector, these levels(catagories) will be replaced by the levels in `to.` This can add levels to a `GRaster` `to` has labels that do not match any existing labels in `from`.
