...
#' @return If \code{grassToR} if \code{TRUE}, then a raster object with the same coordinate reference system, resolution, and extent as \code{rast}. Otherwise, a vector is written into the GRASS session. The name of this vector is as \code{surfFractal}.
...
fasterSurfFractal <- function(
	...
	rgrass7::execGRASS('r.surf.fractal', dimension=dimension, output='surfFractal', flags=flags, ...)

	if (grassToR) {

		out <- rgrass7::readRAST('surfFractal')
		out <- raster::raster(out)
		names(out) <- 'surfFractal'
		out
