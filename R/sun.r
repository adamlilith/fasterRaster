...
#'
#' @param coeff_bh A `GRaster`: A raster with values of the real-sky beam radiation coefficient. Valid values are in the range [0, 1].
#'
#' @param coeff_dh A `GRaster`: A raster with values of the real-sky diffuse radiation coefficient. Valid values are in the range [0, 1].
#' @param slope A `GRaster`: This is a raster representing topographic slope in radians, and it can be calculated using [terrain()].
#'
#' @param aspect A `GRaster`: This is a raster representing topographic aspect in degrees, and it can be calculated using [terrain()]. If calculated with that function, "east orientation" *must* be used (i.e., argument `northIs0` must be `FALSE`).
#'
#' @param hh A "stack" of `GRaster`s: This represents height of the horizon in radians in particular directions. Horizon height can be calculated using [horizonHeight()]. The directions *must* be in "east orientation" (i.e., argument `northIs0` in `horzionHeight()` must be `FALSE`). The directions must correspond with the sequence given by `horizon_step`. For example, if `horizon_step` is 90, then `hh` must contain rasters representing horizon height at 0 (east), 90 (north), 180 (west), and 270 (south) aspects.
#'
#' Note that the `sun()` function uses the [gnames()] of each raster in the stack. The first part of each `gname` must be the same. The last part must be *_xyz*, where *xyz* is an integer between 0 and 359, with leading zeros so the total length of the string *_xyz* is 4 (example: `rast_horizon_q6GW0c35DM_090`).
#' 
...
#'
#' @param time Positive numeric (default is 2): Local solar time. Used for "mode 1" in the `r.sun` **GRASS** module. The default is to use "mode 2".
#'
...
#'
#' @returns A raster or raster stack stack with the same extent, resolution, and coordinate reference system as `elevation`. Assumimg all possible rasters are generated they represent:
