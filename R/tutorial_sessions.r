...
#' To start using **fasterRaster**, a user needs to initiate a "session" using [fastStart()]. This function 1) creates a **GRASS** "location" and "mapset" inside a working directory, and 2) registers **GRASS** so it does operations in this place. Locations and mapsets are stored as folders and files within the working directory, but they also represent data structures in **GRASS**.  Essentially, the working directory contains one or more locations, and each location contains one or more mapsets. A **GRASS** *location* is a set of vectors and/or rasters with the same coordinate reference system (CRS), that may or may not represent the same location on Earth. A **GRASS** location can be thought of as a folder in which spatial objects of the same CRS are stored. *Mapsets* are subfolders inside a location, and typically represent different sub-projects.
#'
#' As used in **fasterRaster** documentation, a *session* is created using [fastStart()] or restored using [fastRestore()]. Having the requisite location and mapset folders and files on disk is not enough. **GRASS** must be started and linked to the location/mapset using one of these functions.
#'
#' ### Working with sessions
#'
#' By default, the working directory is set to the user's temporary folder, obtained through [tempdir()]. However, if a user wishes to save their work, quit **R**, and later restart the **GRASS** session, they will want to use a custom folder. This is specified in [fastStart()] using the argument `workDir`. Restoring a session should be done using [fastRestore()].
#'
...
#'
#' Users can exercise full control over working directory, location, and mapset when they initiate a **GRASS** session using [fastStart()]. The ellipse (`...`) argument allows users to assign a `mapset` and/or `location`, and the `workDir` argument allows for a user-defined working directory.
#'
#' Users can switch between locations, mapsets, and working directories that have been created using [fastRestore()]. If **R** has been restarted and a user wishes to return to a specific (not-temporary) working folder, location, and mapset, then they should use [fastRestore()] (nor `fastStart()`).
#'
#' @keywords tutorial
NULL
