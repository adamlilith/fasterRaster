...
#'
#' As used in **fasterRaster** documentation, a *session* is created using [faster()] or restored using [restoreSession()]. Having the requisite location and mapset folders and files on disk is not enough. **GRASS** must be started and linked to the location/mapset using one of these functions.
#'
#' ### Working with sessions
#'
#' By default, the working directory is set to the user"s temporary folder, obtained through [tempdir()]. However, if a user wishes to save their work, quit **R**, and later restart the **GRASS** session, they will want to use a custom folder. This is specified in [faster()] using the argument `workDir`. Restarting a session should be done using [restoreSession()].
#'
...
#'
#' Users can switch between locations, mapsets, and working directories that have been created using [restoreSession()]. If **R** has been restarted and a user wishes to return to a specific (not-temporary) working folder, location, and mapset, then they should use [restoreSession()] (not `faster()`).
#'
#' @keywords tutorial
NULL
