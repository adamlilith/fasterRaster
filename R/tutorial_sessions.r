...
#'
#' By default, the working directory is set to the user's temporary folder, obtained through [tempdir()]. However, if a user wishes to save their work, quit **R**, and later restart the **GRASS** session, they will want to use a custom folder. This is specified in [faster()] using the argument `workDir`. Restoring a session should be done using [fastRestore()].
#'
...
#'
#' Users can switch between locations, mapsets, and working directories that have been created using [fastRestore()]. If **R** has been restarted and a user wishes to return to a specific (not-temporary) working folder, location, and mapset, then they should use [fastRestore()] (nor `faster()`).
#'
#' @keywords tutorial
NULL
