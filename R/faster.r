...
#'
#' @seealso See the \code{OS Geo GRASS} websit for a catalog and detailed descriptions of \href{https://grass.osgeo.org/grass82/manuals/vector.html}{vector} and \href{https://grass.osgeo.org/grass82/manuals/raster.html}{raster} modules. Related functions include \code{\link[rgrass]{execGRASS}} in \pkg{rgrass}, and \code{\link{fasterRast}} and \code{\link{fasterVect}} for exporting rasters and vectors to an existing \code{GRASS} session.
#'
...
faster <- function(
	...
	if (grassToR) {
		...
		if (pmatch(out, c('raster', 'vector')) == 1L) {
			...
			y <- fasterWriteRaster(outGrassName, paste0(tempfile(), '.tif'), overwrite=TRUE)
