...
This function reports the values in a categorical `GRaster` that have no
matching category label in its "levels" table.
...
- layer:
  ...
  Numeric integers, logical vector, or character: Layer(s) for which to
  obtain missing categories.

## Value

A numeric vector (if `x`is just one layer), or a named list of numeric
vectors, one per layer in `x`.
