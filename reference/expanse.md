...
This function calculates the area of each polygon in a "polygons"
`GVector` or the length of lines in a "lines" `GVector`.
...
- x:
  ...
  A "polygons" or "lines" `GVector`.

- unit:

  Character: Units in which to report values. Areal units are squared,
  linear are not. Can be any of:
