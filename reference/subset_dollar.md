...
The `dollar` notation can be used to get a single layer of a multi-layer
`GRaster` or the values of a column from a `GVector`'s data table.
...
- name:
  ...
  Character: The name of a `GRaster` or of a column of a `GVector`'s
  data table. Names of rasters and vectors can be found using
