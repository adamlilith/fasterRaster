...
The `nacell()` function counts the number of `NA` cells in a `GRaster`,
and the `nonnacell()` reports the number of non-`NA` cells. If the
raster is 3D, then all cells in all layers are counted.
...
- warn:
  ...
  Logical: If `TRUE` (default), display a warning about how much time
  the computation could take.
...
[`terra::ncell()`](https://rspatial.github.io/terra/reference/dimensions.html),
[`dim()`](https://github.com/adamlilith/fasterRaster/reference/dim.md),
[`terra::dim()`](https://rspatial.github.io/terra/reference/dimensions.html)
