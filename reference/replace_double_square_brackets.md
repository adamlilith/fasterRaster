...
The `[[<-` operator can be used to replace a layer in a multi-layer
`GRaster`.
...
- i:
  ...
  A numeric integer, integer, logical, or character: Indicates the layer
  to replace. If a logical vector, then the vector must have the same
  length as there are layers in `x`.

- value:

  Either a `GRaster` or `NULL`: If `NULL`, then the layer indicated by
  `i` will be removed.
