...
`subset()` can be used to subset or remove one or more layers from a
`GRaster`. It can also be used to subset or remove rows from a `GVector`
with a data table.
...
- subset:
  ...
  layer(s) of a `GRaster` to subset, or the rows(s) of a `GVector` to
  return.

- negate:

  Logical: If `TRUE`, all layers or rows in `subset` will be *removed*
