...
This function returns the column names of each "levels" table of a
categorical raster (see
...
- layer:
  ...
  `NULL`, numeric integer, or character: The index (indices) or name(s)
  of one or more raster layers. The default is `NULL`, in which case all
  names for all layers are returned.
