...
`rUnifRast()` creates a raster with values drawn from a uniform (flat)
distribution.
...
- seed:
  ...
  Numeric integer vector or `NULL`: Random seed. If `NULL`, then a
  different seed will be generated by **GRASS**. Defining this is useful
  if you want to recreate rasters. If provided, there should be one seed
  value per raster.
