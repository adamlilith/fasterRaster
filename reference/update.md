...
`GRaster`s and `GVector`s are really pointers to objects in **GRASS**.
The values displayed when you use
...
for a `GRaster` or `GVector` are stored in **R**. If, on the odd chance
that you make a change to a `GRaster` or `GVector` (e.g., using commands
in the **rgrass** package), the changes will not be automatically
reflected in the `GRaster` or `GVector`. This function can be used to
update the objects in **R** to reflect their proper values.
