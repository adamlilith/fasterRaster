...
max. value  :     570
```
Converting rasters and vectors that are already in **R** to `GRaster`s and `GVector`s usually takes more time than loading them directly from disk. To load from disk, simply replace the first argument in [fast()] with a string representing the folder path and file name of the raster you want to load into the session. For example, you can do:
...
plot(riversBuff, col = 'white', add = TRUE)
```

![Distance to rivers](dist_to_rivers.png")
