...
```
**GRASS** comes with a default set of modules (functions), but users can download and install "addon" modules to extend its capabilities. Some of the functions in **fasterRaster** rely on these addon modules. To make these functions work, you must find the folder in which addons are stored and install the desired tool.
...
## Validating addons
You can test to see if the `addons` folder is correct using `addons()`. You can also see if a particular addon tool was installed correctly using the same function using, for example, `addons("v.centerpoint")`.
