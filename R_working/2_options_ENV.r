...
#'
#' These functions allows you to either 1) view or 2) define options shared by **fasterRaster** functions. You can view the current option(s) using, for example:
#' ```
...
#' ```
#' setFastOptions(rastClass = 'stars', vectClass = 'sf', memory = 600, cores = 4)
#' ```
...
#'
#' @return The `getFastOptions()` function returns a list of the value of one or more **fasterRaster** settings. The `setFasterOptions()` function changes the values of these settings and returns the pre-existing values as a list (invisibley--i.e., so you can revert to them if you want).
#'
...
setFastOptions <- function(
	...
	if (length(dots) > 0L & restore) {
		...
		### set the options
		if (!exists('.fasterRaster', emptyenv())) .fasterRaster <- new.env(parent = emptyenv())
		...
		fasterRaster <- options('fasterRaster')
		if (is.null(fasterRaster)) fasterRaster <- list()
		...
		for (opt in names(dots)) {
			...
			if (opt %in% names(fasterRaster)) {
				fasterRaster[[opt]] <- val
			} else {
				fasterRaster[[length(fasterRaster) + 1L]] <- val
				names(fasterRaster)[length(fasterRaster)] <- opt
			}
		
		}
		
		options(fasterRaster = fasterRaster)
...
getFastOptions <- function(..., default = FALSE) {
	...
	opts <- c(...)
	if (length(opts) > 0L && any(!(opts %in% namesOfOpts))) stop('Invalid option(s): ', paste(opts[!(opts %in% namesOfOpts)], collapse=', '))
	...
	### return options as they currently are
		...
		out <- list()
		fr <- getOption('fasterRaster', default = NULL)
		
		# we have no options :(
		if (length(fr) == 0L) {
			out <- getFastOptions(default = TRUE)
		# we have options, people!
			...
			for (opt in opts) {
				...
				thisOptValue <- fr[[opt]]
				if (is.null(thisOptValue)) {
					default <- paste0('.', opt, 'Default()')
					default <- eval(str2expression(default))
					out[[length(out) + 1L]] <- default
				} else {
					out[[length(out) + 1L]] <- thisOptValue
				}

			}
			names(out) <- opts
			
		} # see? we have options, people!
